...
private:
  ...
  bool verifyJobsAntecedence() {
    ...
    for (int i = 0; i < J; i++) {
      if (!verifyjobAntedecende(jobOrder[i]))
        return false;
    }
    return true;
  }
  ...
  void generateMachineOrder(vector<Job> machine,
                            unsigned mI /*Machine index*/) {}
  ...
  bool verifyMachinesSchedule() {
    ...
    for (int i = 0; i < M; i++) {
      if (!verifyMachineSchedule(machineOrder[i]))
        return false;
    }
    return true;
  }
...
public:
  ...
  bool isViable() {
    ...
    if (jobAntecedence && machineorder)
      cout << "OK!";
    else {
      cout << "ERROR";
      if (!jobAntecedence)
        cout << "-JOB STARTS BEFORE ITS PREDECESSOR ENDS";
      if (!machineorder)
        cout << "-TWO JOBS USING THE SAME MACHINE AT SAME TIME";
    }
    cout << endl;
